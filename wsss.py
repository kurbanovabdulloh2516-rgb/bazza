...
# # cousor.execute("""
# # UPDATE abdullokh 
# # SET name=?, surname=?, age=?, number=?, class=?, email=?, parents_phone=?, school=? 
# # WHERE id=?
# # """, ('abullokh', 'kurbanov', 14, '0557747476', '9B',
# #       'kurbanov.abdulloh.2516@gmail.com', '0551037111', 'SEMA', 5))

# # connect.commit()


# # cousor.execute('SELECT * FROM abdullokh')
# # users = cousor.fetchall()
# # for user in users:
# #     print(user)

# # connect.commit()

# cousor.execute("DELETE FROM abdullokh WHERE id=?", (3 ,))
# connect.commit()
# cousor.execute("DELETE FROM abdullokh WHERE id=?", (6 ,))
# connect.commit()